...
&НаКлиенте
Процедура ДекорацияДанныеАвторизацииСохраненыОбработкаНавигационнойСсылки(Элемент, НавигационнаяСсылкаФорматированнойСтроки, СтандартнаяОбработка)
...
Процедура ОбновитьДанныеПодключенияНаСервере()
	...
	Если ЗначениеЗаполнено(ВерсияGit) Тогда
		Элементы.КомментарийПутьКGit.Заголовок = Новый ФорматированнаяСтрока(ВерсияGit, , Новый Цвет(, 255, ));
		Элементы.ПодключениеИнициализация.Доступность = Истина;
	Иначе
		Элементы.КомментарийПутьКGit.Заголовок = Новый ФорматированнаяСтрока(ОшибкаGit, , Новый Цвет(255, , ));
	...
	Если Не НастройкаПодключения.Пустая() Тогда
		...
		Если НастройкаПодключения.Инициализирован Тогда
			Элементы.НадписьРезультатПодключения.Заголовок = 
				Новый ФорматированнаяСтрока("Репозиторий подключен", , Новый Цвет(, 255, ));
		Иначе
			Если ЗначениеЗаполнено(ОшибкаИнициализации) Тогда
				Элементы.НадписьРезультатПодключения.Заголовок = 
					Новый ФорматированнаяСтрока(ОшибкаИнициализации, , Новый Цвет(255, , ));
			Иначе
...
Процедура ПроверитьПодключениеНаСервере()
	Главный = "master";
	НастройкаПодключения = Справочники.РГНастройкиРепозиториев.НайтиПоНаименованию("master");
	...
	ИначеЕсли Не РасположениеУзлаАктуально Тогда
		...
		МассивСтроки.Добавить("Подключение узла не актуально. Узел подключен для расположения ");
...
Процедура ДанныеРепозиторияПриИзмененииНаСервере()
	...
	Если ИзменитьСохраненные Тогда
		...
		ИначеЕсли СпособАвторизации = Перечисления.РГСпособыАвторизации.OAuth И ЗначениеЗаполнено(Токен) Тогда
	...
	Если Результат.Успешно Тогда
		...
		Элементы.НадписьРезультатПодключения.Заголовок = 
			Новый ФорматированнаяСтрока("Репозиторий подключен", , Новый Цвет(, 255, ));
	Иначе
		ОшибкаИнициализации = Результат.Ошибка;
		Элементы.НадписьРезультатПодключения.Заголовок = 
			Новый ФорматированнаяСтрока(ОшибкаИнициализации, , Новый Цвет(255, , ));
...
&НаСервере
Процедура ЗавершитьПодключениеНаСервере(МассивКонтуров)
	Главный = "master";
	НастройкаПодключения = Справочники.РГНастройкиРепозиториев.НайтиПоНаименованию(Главный);
	...
	Путь = КШДЯдроВызовСервера.ПолучитьПутьКФайлам(Главный);
	...
	Запись.ОткрытьФайл(Путь + "nodes.json");
