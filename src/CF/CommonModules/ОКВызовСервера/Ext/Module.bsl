...
Функция ВыполнитьОбновлениеДанных() Экспорт
	ТекущаяВерсияДанных = Константы.ОКТекущаяВерсияДанных.Получить();
	Если ЧислоРелиза(ТекущаяВерсияДанных) < ЧислоРелиза(Метаданные.Версия) Тогда
		Попытка
			ВыполнитьНовыеОбработчики();
			Константы.ОКТекущаяВерсияДанных.Установить(Метаданные.Версия);
		Исключение
...
Процедура ВыполнитьНовыеОбработчики()
	МассивОбработчиков = ОКНастройкаОбновленияСервер.ПолучитьСписокОбработчиков();
	
	Для Каждого ДанныеОбработчика Из МассивОбработчиков Цикл
		Рег = РегистрыСведений.ОКВыполненныеОбработчикиОбновления.СоздатьМенеджерЗаписи();
		...
		Если Не Рег.Выбран() Тогда
			...
			ОбщегоНазначенияВызовСервера.ПроверитьИмяПроцедурыКонфигурации(ДанныеОбработчика.Обработчик);		
