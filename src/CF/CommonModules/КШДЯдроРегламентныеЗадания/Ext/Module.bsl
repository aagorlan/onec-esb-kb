	...
	Пока Истина Цикл
		...
		Пока ВыборкаЗаданий.Количество() < Потоков Цикл
			...
			Если РезультатЧтения.ЕстьСообщение Тогда
				...
				МассивПараметров[0] = ТекущаяУниверсальнаяДатаВМиллисекундах();
				МассивПараметров[3] = Перечисления.КШДЯдроВидыСобытий.НачалоОбработки;
				ФоновыеЗадания.Выполнить("КШДЯдроСервер.ЗафиксироватьСобытиеОчередиВФоне", МассивПараметров);
			КонецЕсли;
		КонецЦикла;
	КонецЦикла;
...
// Анализирует и сворачивает данные статистики показателей
//
Процедура КШДЯдроСверткаПоказателейРаботы() Экспорт
	Запрос = Новый Запрос;
	Запрос.Текст = "ВЫБРАТЬ
	               |	КШДЯдроПоказателиРаботыУзла.Период КАК СтарыйПериод,
	               |	ВЫБОР
	               |		КОГДА РАЗНОСТЬДАТ(КШДЯдроПоказателиРаботыУзла.Период, &ТекущаяДата, МИНУТА) < 60
	               |			ТОГДА КШДЯдроПоказателиРаботыУзла.Период
	               |		ИНАЧЕ НАЧАЛОПЕРИОДА(КШДЯдроПоказателиРаботыУзла.Период, ЧАС)
	               |	КОНЕЦ КАК Период,
	               |	КШДЯдроПоказателиРаботыУзла.Показатель КАК Показатель,
	               |	КШДЯдроПоказателиРаботыУзла.Аналитика КАК Аналитика,
	               |	КШДЯдроПоказателиРаботыУзла.Число КАК Число,
	               |	КШДЯдроПоказателиРаботыУзла.Строка КАК Строка,
	               |	КШДЯдроПоказателиРаботыУзла.Дата КАК Дата,
	               |	КШДЯдроПоказателиРаботыУзла.Булево КАК Булево
	               |ИЗ
	               |	РегистрСведений.КШДЯдроПоказателиРаботыУзла КАК КШДЯдроПоказателиРаботыУзла
	               |ГДЕ
	               |	КШДЯдроПоказателиРаботыУзла.Период <> ВЫБОР
	               |			КОГДА РАЗНОСТЬДАТ(КШДЯдроПоказателиРаботыУзла.Период, &ТекущаяДата, МИНУТА) < 60
	               |				ТОГДА КШДЯдроПоказателиРаботыУзла.Период
	               |			ИНАЧЕ ВЫБОР
	               |					КОГДА РАЗНОСТЬДАТ(КШДЯдроПоказателиРаботыУзла.Период, &ТекущаяДата, МИНУТА) < 60 * 24
	               |						ТОГДА НАЧАЛОПЕРИОДА(КШДЯдроПоказателиРаботыУзла.Период, ЧАС)
	               |					ИНАЧЕ НАЧАЛОПЕРИОДА(КШДЯдроПоказателиРаботыУзла.Период, ДЕНЬ)
	               |				КОНЕЦ
	               |		КОНЕЦ
	               |ИТОГИ
	               |	МАКСИМУМ(Число),
	...
	ВыборкаПериодов = Запрос.Выполнить().Выбрать(ОбходРезультатаЗапроса.ПоГруппировкам, "Период");
	Пока ВыборкаПериодов.Следующий() Цикл
		ВыборкаПоказателей = ВыборкаПериодов.Выбрать(ОбходРезультатаЗапроса.ПоГруппировкам, "Показатель");
		Пока ВыборкаПоказателей.Следующий() Цикл
			ВыборкаАналитик = ВыборкаПоказателей.Выбрать(ОбходРезультатаЗапроса.ПоГруппировкам, "Аналитика");
			Пока ВыборкаАналитик.Следующий() Цикл
				Рег = РегистрыСведений.КШДЯдроПоказателиРаботыУзла.СоздатьМенеджерЗаписи();
				ЗаполнитьЗначенияСвойств(Рег, ВыборкаАналитик);
				Рег.Записать(Истина);
				Выборка = ВыборкаАналитик.Выбрать();
				Пока Выборка.Следующий() Цикл
					Рег = РегистрыСведений.КШДЯдроПоказателиРаботыУзла.СоздатьМенеджерЗаписи();
					Рег.Период = Выборка.СтарыйПериод;
					Рег.Показатель = Выборка.Показатель;
					Рег.Аналитика = Выборка.Аналитика;
					Рег.Прочитать();
					Рег.Удалить();
				КонецЦикла;
			КонецЦикла;
